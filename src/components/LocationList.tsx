...
const LocationList: React.FC<LocationListProps> = ({
    ...
    if (!locationName || locationName === undefined) {
        return <>locationName is missing</>;
    }

    if (!correctGuesses) {
        return <>correctGuesses is missing</>;
    }

    if (!submitted) {
        return <>submitted is missing</>;
    }

    if (!typedText) {
        return <>typedText is missing</>;
    }

    if (!currentLocationStep && typeof currentLocationStep !== "number") {
        return <>currentLocationStep is missing</>;
    }
