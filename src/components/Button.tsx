...
interface ButtonProps {
    btnName: string;
    btnColor: 'retro-orange' | 'retro-yellow' | 'retro-mint' | 'retro-purple' | 'retro-bg' | 'sky' | 'emerald' | 'red' | 'gray';
    event?: () => void | null;
}
...
const Button: React.FC<ButtonProps> = ({ btnName, btnColor, event }) => {
    ...
    return (
        <>
            <button onClick={event || undefined} className={`w-full px-2 py-3  text-white bg-gradient-to-b ${gradient} hover:opacity-90 relative`}>
                {btnName}
            </button>
        </>
    )
